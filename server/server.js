...
app.post('/edit', authenticate, (req, res) => {
  ...
  const editProcess = spawn('blender', [
    '--background',
    '--python', path.join(__dirname, 'blender-scripts', 'edit_properties.py'),
...
app.post('/export', authenticate, (req, res) => {
  const { fileName } = req.body;

  // Call the Blender script to export as GLB
  const exportProcess = spawn('blender', [
    '--background',
    '--python', path.join(__dirname, 'blender-scripts', 'export_glb.py'),
    '--', fileName
  ]);
  ...
  exportProcess.on('close', (code) => {
    if (code === 0) {
      res.status(200).sendFile(path.join(__dirname, 'uploads', `${fileName}.glb`));
    } else {
